...
public class StrategieDeCombatSpatial implements Serializable {
 ...
 public String[] getTypesDeVaisseau(){return (String[])positionnement.keySet().toArray(new String[0]);}
 ...
 public int getAgressivite(){return agressivite;}
 public int getTypeCible(){return typeCible;}
 ...
 public static boolean[] estStrategieValide(Commandant c,int agress,int typeC,String[] vaisseau,int[][] pos,int[][] tC){
  if(agress>Const.STRATEGIE_AGRESSIVITE_NB_MAXIMAL){
   Univers.ajouterErreur(c.getNomNumero(),"ER_STRATEGIE_COMBAT_0000",agress);
   return null;
   }
  if(typeC>Const.STRATEGIE_AGRESSIVITE_NB_MAXIMAL){
   Univers.ajouterErreur(c.getNomNumero(),"ER_STRATEGIE_COMBAT_0001",typeC);
   return null;
   }
  ...
  for(int i=0;i<vaisseau.length;i++){
   ...
   if(!Univers.existencePlanDeVaisseau(vaisseau[i])){
    Univers.ajouterErreur(c.getNomNumero(),"ER_STRATEGIE_COMBAT_0002",vaisseau[i]);
    ok=false;
    }
   if((pos[i][0]>Const.COMBAT_X_MAX)||(pos[i][1]>Const.COMBAT_Y_MAX)){
    Univers.ajouterErreur(c.getNomNumero(),"ER_STRATEGIE_COMBAT_0003",vaisseau[i],pos[i][0],pos[i][1]);
    ok=false;
    }
   boolean bon=true;
   for(int j=1;j<11;j++)
    if(!Mdt.estPresent(tC[i],j)) bon=false;
   if(!bon){
 ...
 public StrategieDeCombatSpatial(String n,int agress,int typeCib,String[] typeDeVaisseau,int[][] place,int[][] cibles){
  nom=n;
  agressivite=agress;
  typeCible=typeCib;
