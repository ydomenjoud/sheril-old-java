...
public class Gouverneur extends Leader implements Serializable {
	...
	public static transient Gouverneur GOUVERNEUR_NON_PRESENT = new Gouverneur(
	...
	public boolean estEnReserve() {
		if (position == Const.POSITION_GOUVERNEUR_RESERVE)
			return true;
		else
			return false;
	}
	...
	public String descriptionPosition(Locale l) {
		if (estEnReserve())
			return Utile.maj(Univers.getMessage("RESERVE_LEADER", l));
		else
			return Utile.maj(Univers.getMessage("POSITION_GOUVERNEUR", l))
					+ Univers.getSysteme(position).getNomPosition(l);
	}
	...
	public Gouverneur(String n, int[][] comp, int vit, int att, int def,
			int mor, int march, int rac, int tour) {
