...
//un arbre de commentaires  un niveau.
		...
		private String traduction(Locale l, int type) {
			...
			for (int i = 0; i < parametres.length; i++){
				...
				if( parametres[i] == null ){
					System.out.println("parametre: "+i+" => " + textePrincipal);
					System.exit(0);
				}
			...
			if (type == Const.MESSAGE_TYPE_COMMANDANT)
				...
				if (Univers.existenceMessageSysteme(textePrincipal))
					phrase = Univers.getMessageSysteme(textePrincipal) + " ({"
							+ Integer.toString(parametres.length - 1) + "})";
				else
					phrase = textePrincipal;
